...
`evaluate` runs labeled cases with fresh temporary workspaces and an explicit suite budget. `score` grades imported observations without a provider or key. Both write portable reports and enforce a configurable pass threshold; missing cases and unknown billing remain visible. See [evaluation.md](evaluation.md) for formats, provenance and CLI examples.
