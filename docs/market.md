...
## Early implementation evidence, not a market benchmark

The repository now includes 36 authored synthetic evaluation cases and a scorer for Flock or imported outputs. Deterministic example baselines passed 11/16 triage cases, 11/12 common-log diagnosis cases and 8/8 mechanical release-note checks, with no model calls. The corpora are deliberately small and include selected edge cases; these rates are not production estimates or evidence that Flock outperforms an incumbent. The documentation score measures citation/structure constraints, not editorial quality. These observations support a specific discipline: establish a cheap baseline first and require evidence before assigning work to a model. See [methods and limitations](evaluation.md).
