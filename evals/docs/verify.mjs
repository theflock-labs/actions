...
const text = await readFile('release-notes.md', 'utf8');
...
if (!text.startsWith('# ') || text.length > 10000 || actual.size !== expected.size || [...expected].some(id => !actual.has(id))) {
  throw new Error('Release notes must have a heading, stay under 10,000 characters, and cite every supplied change ID with no invented IDs.');
}
